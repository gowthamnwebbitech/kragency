        ...
        <div class="container">
            <div class="row">
                <div class="col-lg-12">
                    <div class="lottery-times">
                        @foreach($schedules as $schedule)
                            ...
                            @if($scheduleTime->greaterThan($compareTime))
                                <div onclick="window.location.href='{{ route('customer.play-now', ['id' => $schedule->betting_providers_id, 'time_id' => $schedule->id]) }}'" style="cursor:pointer;" class="lottery-card {{ $scheduleTime->lessThan($compareTime) ? 'closed' : ($schedule->id == $slot_time_id ? 'active' : 'running') }}">
                            @else
                                <div  class="lottery-card {{ $scheduleTime->lessThan($compareTime) ? 'closed' : ($schedule->id == $slot_time_id ? 'active' : 'running') }}">
                            @endif
                    ...
                    @if($show_slot == 1)
                        @foreach($gameSlots as $key => $game_slots)
                            @if($key == 1)
                                <div class="game-box singleDigit">
                            @elseif($key == 2)
                                <div class="game-box singleDigit">
                            @elseif($key == 3)
                                <div class="game-box">
                            ...
                            @endif
                                ...
                                @foreach($game_slots as $game_slot)
                                    <div class="row">
                                        <div class="col-md-12 p-0">
                                            <div class="header">
                                                @if($loop->first)
                                                    <div>
                                                        @if($game_slot->digitMaster->type == 1)
                                                            <div class="title">Single Digit <span>Win ₹{{ $game_slot->providerSlot->winning_amount }}</span></div>
                                                            <div class="price">₹{{ $game_slot->amount }}</div>
                                                        @elseif($game_slot->digitMaster->type == 2)
                                                            <div class="title">Double Digits <span>Win ₹{{ $game_slot->providerSlot->winning_amount }}</span></div>
                                                            <div class="price">₹{{ $game_slot->amount }}</div>
                                                        @elseif($game_slot->digitMaster->type == 3)
                                                            <div class="title">Three Digits <span>Win ₹{{ $game_slot->providerSlot->winning_amount }}</span></div>
                                                            <div class="price">₹{{ $game_slot->amount }}</div>
                                                        @elseif($game_slot->digitMaster->type == 4)
                                                            <div class="title">Four Digits <span>Win ₹{{ $game_slot->providerSlot->winning_amount }}</span></div>
                                                            <div class="price">₹{{ $game_slot->amount }}</div>
                                                        @endif
                                    ...
                                    @if($key == 1)
                                        <div class="gridWrap" data-type="1" data-game-label="{{ $game_slot->digitMaster->name }}" data-game-id="{{ $game_slot->id }}" data-amount="{{ $game_slot->amount }}">
                                            <div class="gridChild">
                                                <div class="label">{{ $game_slot->digitMaster->name }}</div>
                                            </div>
                                            <div class="gridChild">
                                                <input type="text" class="input-box" maxlength="1" name="digit">
                                            </div>
                                            <div class="gridChild">
                                                <div class="d-flex">
                                                    <div class="counter">
                                    ...
                                    @elseif($key == 2)
                                        <div class="gridWrap" data-type="2" data-game-label="{{ $game_slot->digitMaster->name }}" data-game-id="{{ $game_slot->id }}" data-amount="{{ $game_slot->amount }}">
                                            <div class="gridChild">
                                                <div class="d-flex">
                                                    @php
                                                        $string_slot = $game_slot->digitMaster->name;
                                                        $slots = str_split($string_slot);
                                                    @endphp
                                            ...
                                            <div class="gridChild">
                                                <div class="d-flex">
                                                    <input type="text" class="input-box" maxlength="1" name="digit1">
                                                    <input type="text" class="input-box" maxlength="1" name="digit2">
                                                </div>
                                            </div>
                                            <div class="gridChild">
                                                <div class="d-flex">
                                    ...
                                    @elseif($key == 3)
                                    @if(in_array($game_slot->digitMaster->id, [9, 10]))
                                        @continue
                                    @endif
                                        <div class="gridWrap gridThree" data-type="3" data-game-label="{{ $game_slot->digitMaster->name }}" data-game-id="{{ $game_slot->id }}" data-amount="{{ $game_slot->amount }}">
                                            <div class="gridChild">
                                                <div class="d-flex">
                                                    @php
                                                        $string_slot = $game_slot->digitMaster->name;
                                            ...
                                            <div class="gridChild">
                                                <div class="d-flex justifyEnd">
                                                    <input type="text" class="input-box" maxlength="1" name="digit1">
                                                    <input type="text" class="input-box" maxlength="1" name="digit2">
                                                    <input type="text" class="input-box" maxlength="1" name="digit3">
                                                </div>
                                            </div>
                                            <div class="gridChild">
                                                <div class="d-flex">
                                    ...
                                    @elseif($key == 4)
                                    @if(in_array($game_slot->digitMaster->id, [9, 10, 11]))
                                        @continue
                                    @endif
                                        <div class="gridWrap gridThree" data-type="4" data-game-label="{{ $game_slot->digitMaster->name }}" data-game-id="{{ $game_slot->id }}" data-amount="{{ $game_slot->amount }}">
                                            <div class="gridChild">
                                                <div class="d-flex">
                                                    @php
                                                        $string_slot = $game_slot->digitMaster->name;
                                            ...
                                            <div class="gridChild">
                                                <div class="d-flex">
                                                    ...
                                                    <input type="text" class="input-box" maxlength="1" name="digit2">
                                                    <input type="text" class="input-box" maxlength="1" name="digit3">
                                                    <input type="text" class="input-box" maxlength="1" name="digit4">
                                                </div>
                                            </div>
                                            <div class="gridChild">
                                                <div class="d-flex">
...
<script>
    $(document).ready(function() {
        ...
        $('.add-to-cart').click(function() {
            ...
            if (type == 1) {
                ...
                if (digit1 === '' || isNaN(digit1) || digit2 === '' || isNaN(digit2) || digit3 === '' || isNaN(digit3)) {
                    alert('Please enter valid digits');
                    isValid = false;
                } else {
            ...
            let newTotal = cart.reduce(function(sum, item) { return Number(sum) + Number(item.total || 0); }, 0) + Number(cartItem.total);
                ...
                success: function(response) {
                    if (response.success) {
                        ...
                        $.ajax({
                            ...
                            success: function(response) {
                                if (response.success) {
                                    cart = response.cart; // updated cart from server
                                    $('#cartCount').text(cart.length);
                                    Swal.fire({
                                        icon: 'success',
                                        title: 'Added to cart!',
                                    ...
                                    if (isBox) gameWrapper.find('.box-toggle').removeClass('active');
                                } else {
                        ...
                        alert(response.message || 'Insufficient wallet balance. Item not added to cart.');
                        // Defensive: if you ever push before check, remove last
                        if (cart.length > 0) {
                            let last = cart[cart.length-1];
                            if (last && last.game_id === cartItem.game_id && last.digits === cartItem.digits && last.amount === cartItem.amount && last.quantity === cartItem.quantity) {
                                cart.pop();
                                $('#cartCount').text(cart.length);
                            }
                        }
    ...
    document.addEventListener('DOMContentLoaded', function() {
        ...
        document.querySelectorAll('.gridWrap[data-type="2"] .input-box, .gridWrap[data-type="3"] .input-box, .gridWrap[data-type="4"] .input-box').forEach(function(input, idx, arr) {
            input.addEventListener('input', function(e) {
                if (e.target.value.length === e.target.maxLength) {
                    // Find the next input in the same parent
                    let next = e.target.parentElement.querySelectorAll('.input-box')[Array.prototype.indexOf.call(e.target.parentElement.querySelectorAll('.input-box'), e.target) + 1];
                    if (!next) {
                        // Try to find next input in the next sibling (for flex layouts)
                        let allInputs = Array.from(e.target.closest('.d-flex, .d-flex.justifyEnd').querySelectorAll('.input-box'));
